...
TableExtension 50101 "tableextension50101" extends Employee
{
    fields
    {
        ...
        */
        ...
        {
            OptionCaption = 'APHF,ACDC,APHF Intern, APHF-MU,Interim';
            OptionMembers = APHF,ACDC,"APHF Intern","APHF-MU",Interim;
        }
        ...
        field(69600; "Current Position ID"; Code[30])
        {
            ...
            trigger OnValidate()
            var
                Pos: Record "Company Positions";
            begin
                if Pos.Get("Current Position ID") then
                    "Job Title" := Pos."Job Title";
        ...
        {
            CalcFormula = sum("HR Leave Ledger Entries"."No. of days" where("Leave Posting Type" = const("Off Days"),
                                                                             "Staff No." = field("No.")));
            FieldClass = FlowField;
        }
        ...
        {
            DataClassification = ToBeClassified;
            TableRelation = "Dimension Value".Code where("Dimension Code" = const('DIV/UNIT'));
        }
        ...
        {
            DataClassification = ToBeClassified;
            OptionCaption = 'Employee,Intern,ACDC';
            OptionMembers = Employee,Intern,ACDC;
        }
        ...
        field(70063; "Primary Swift Code"; Code[20])
        {
            TableRelation = "SWIFT Code".Code;
            DataClassification = ToBeClassified;
        }
        field(70064; "Bank 2 Swift Code"; Code[20])
        {
            TableRelation = "SWIFT Code".Code;
            DataClassification = ToBeClassified;
        }
        field(70065; Selfcare; Decimal)
        {
            CalcFormula = sum("HR Leave Ledger Entries"."No. of days" where("Leave Posting Type" = const(Selfcare),
                                                                             "Staff No." = field("No.")));
            FieldClass = FlowField;
        }
        field(70066; "Exchange Rate"; Blob)
        {
            DataClassification = ToBeClassified;
        }
        field(70067; "Advance Account"; Code[200])
        {
            TableRelation = Customer."No." where("Funding Class" = const(Employee));
        }
        //
        field(70068; "SHIF Amount"; Decimal)
        {
            CalcFormula = - sum("Assignment Matrix-X".Amount where(Type = const(Deduction),
                                                                  "Employee No" = field("No."),
                                                                  "Payroll Period" = field("Pay Period Filter"),
                                                                  "SHIF Code" = const(true)));
            FieldClass = FlowField;

        }
        field(70069; "Housing Fund Amount"; Decimal)
        {
            CalcFormula = - sum("Assignment Matrix-X".Amount where(Type = const(Deduction),
                                                                  "Employee No" = field("No."),
                                                                  "Payroll Period" = field("Pay Period Filter"),
                                                                  "Housing Fund Code" = const(true)));
            FieldClass = FlowField;
        }
        ...
        field(70070; "Exclude Housing"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(70071; "Exclude SHIF"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
    ...
    */
    ...
    begin
        if not ResourceRec.Get("No.") then begin
            // if "Employee Type" = "Employee Type"::APHF then begin
            ResourceRec."No." := "No.";
    ...
    begin
        if not CustRec.Get("No.") then begin
            ...
            // CustRec."No." := "No.";
            // CustRec."No." := NoSeriesManagement.GetNextNo(SalesSetup."Customer Nos.", Today, true);
            CustRec."No." := NoSeriesManagement.GetNextNo(SalesSetup."Employee Advance No.", Today, true);
    ...
    begin
        ...
        "No." := NoSeriesMgt.GetNextNo(HumanResSetup."Employee Nos.", WorkDate(), true);
