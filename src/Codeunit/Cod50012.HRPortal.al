...
Codeunit 50012 "HRPortal"
{
    ...
    trigger OnRun()
    begin
        //MESSAGE(generatePayslip('004',CURRENTDATETIME));
        //MESSAGE(fnGetSubPlogLines('PLOG0096','ACTIVITY1.4.1.1','ISCRD0141'));
        //MESSAGE(fnSuggestPlogandSubPlogData('PLOG0096'));
        //MESSAGE(generateEmployeeStatement('004',20210101D,20220101D));
        sendLeaveForApproval('APHF0001', 'LA000007')
        // Message(fnCreateF());
    end;
    ...
    var
        ...
        FILESPATH1: label 'C:\inetpub\wwwroot\EssPortalTest\downloads\P9Payslip\';
        FILESPATH2: label 'C:\inetpub\wwwroot\EssPortalTest\downloads\P9\';
        ...
        FILESPATH13: label 'C:\inetpub\wwwroot\ESSTEST\downloads\RiskReport\';
        FILESPATH14: label 'C:\inetpub\wwwroot\ESSTEST\downloads\StandardAppraisalReport\';
        FILESPATH15: label 'C:\inetpub\wwwroot\ESSTEST\downloads\PlogReport\';
        FILESPATH16: label 'C:\inetpub\wwwroot\ESSTEST\downloads\IndividualPCReport\';
        FILESPATH17: label 'C:\inetpub\wwwroot\ESSTEST\downloads\CSP\';
        FILESPATH18: label 'C:\inetpub\wwwroot\ESSTEST\downloads\AWP\';
        FILESPATH19: label 'C:\inetpub\wwwroot\ESSTEST\downloads\BoardPC\';
        FILESPATH20: label 'C:\inetpub\wwwroot\ESSTEST\downloads\TrainingFeedback\';
        FILESPATH21: label 'C:\inetpub\wwwroot\ESSTEST\downloads\TrainingEvaluation\';
        FILESPATH22: label 'C:\inetpub\wwwroot\ESSTEST\downloads\TrainingApplication\';
        FILESPATH23: label 'C:\inetpub\wwwroot\ESSTEST\downloads\TrainingPlan\';
        FILESPATH24: label 'C:\inetpub\wwwroot\ESSTEST\downloads\IndividualSubIndicator\';
        FILESPATH25: label 'C:\inetpub\wwwroot\ESSTEST\downloads\ScoreReport\';
        ...
        CMUTUA: Label 'CMUTUA';
        ECHEGE: Label 'ECHEGE';
        EOKEREKE: Label 'EOKEREKE';
        FKIMONYI: Label 'FKIMONYI';
        HJELLE: Label 'HJELLE';
        JMBOWA: Label 'JMBOWA';
        JOHNBRIAN: Label 'JOHNBRIAN';
        MBIESEBROEK: Label 'MBIESEBROEK';
        PARITO: Label 'PARITO';
        PBABU: Label 'PBABU';
        SKIMANI: Label 'SKIMANI';
        VKUBENDE: Label 'VKUBENDE';
        ERP: Label 'JOHN';
        UsId: Code[50];
        SequenceNo: Integer;
        ApproverId: Code[50];
    ...
    begin
        ...
        if employeeEmail = '' then begin
            ...
            // SMTPMailSetup.Get();           
            // SMTPMail.Create(employeeEmail, 'Employee Password Reset', 'Your one time password is <strong>' + Format(password) + '</strong>', true);
            ...
            Body := 'Dear, ' + Employee.FullName() + ' your password for the account: ' + ' <strong>' + Employee."Company E-Mail" + '</strong> has been created/reset successfully. Kindly use the password below to access your account<br> <strong>Password: ' + HRPortalUsers.password + '</strong> <br>';
            Body := Body + ' ' + 'Kind Regards, Africa Public Health Foundation' + '<br><br>';
            Body := Body + '[This email is automated. Kindly do not reply to it]<br><br>';
            SMTPMail.Create(Employee."Company E-Mail", Subject, Body, true);
            Smail.Send(SMTPMail, Enum::"Email Scenario"::Default);
    ...
    begin
        ...
        Payments.SetRange("No.", docNo);
        Payments.SetRange("Account No.", empNo);
        Payments.SetRange(Status, Payments.Status::Approved);
        if Payments.FindSet then begin
    ...
    begin
        ...
        if Payments.FindSet then begin
            ...
            if Payments.Get(reqNo) then begin
    ...
    begin
        ...
        if Employee.FindSet then begin
            ...
            if Employee.FindSet then begin
                ...
                Report.SaveAs(89028, '', ReportFormat::Pdf, OutStr, RecRef);
    ...
    begin
        ...
        if Employee.FindSet then begin
            RecRef.GetTable(Employee);
            TempBlob.CreateOutStream(OutStr);
            Report.SaveAs(69019, '', ReportFormat::Pdf, OutStr, RecRef);
            TempBlob.CreateInStream(InStr2);
            //InStr2.ReadText(BodyTxt);
            base64String := cuBase64.ToBase64(InStr2);
            //exit(base64String);
    ...
    begin
        ...
        if imprestNo = '' then begin
            ...
            if Employee.FindSet then
                ImprestMemo."Responsibility Center" := Employee.Region;
            ImprestMemo."Shortcut Dimension 2 Code" := Employee."Global Dimension 2 Code";
    ...
    procedure addTeamMember(employeeNo: Code[50]; imprestNo: Code[50]; destinationTown: Text; traveltype: Option; voteItem: Text; teamMember: Text; noOfDays: Decimal) status: Text
    var
    ...
    begin
        ...
        If tbl_employee.FindFirst() Then begin
            ...
            IF tbl_payments.Findset(true) then begin
                ...
                tbl_payments.TestField("Payment Narration");
                tbl_payments.TestField("Shortcut Dimension 1 Code");
                tbl_payments.TestField("Shortcut Dimension 2 Code");
                tbl_payments.TestField("Shortcut Dimension 3 Code");
                if tbl_payments."Imprest Amount" = 0 then
                    Error(Text001, tbl_payments."No.");

                //Check for Unsurrendered Impres
                // ImpH.Reset();
                // ImpH.SetRange("Payment Type", ImpH."Payment Type"::Imprest);
                // ImpH.SetRange("Account No.", tbl_payments."Account No.");
                // ImpH.SetRange(Status, ImpH.Status::Released);
                // ImpH.SetFilter("No.", '<>%1', tbl_payments."No.");
                // ImpH.Calcfields("Imprest Amount");
            ...
            status := 'danger*Sorry, you dont have  an Advance Account.Kindly contact ICT';
        end;

    END;

    procedure cancelImprestRequisitionApproval(employeeNo: Code[50]; imprestNo: Code[50]) status: Text
    var
        tbl_payments: Record Payments;
        ApprovalsMgmt: Codeunit "Approvals Mgmt.";
        VarVariant: Variant;
    ...
    begin
        ...
        IF tbl_payments.Findset(true) then begin
            ...
            status := 'success*Your request was successfully  cancelled your approval request.';
        end else begin
            status := 'danger*Sorry, your imprest has already been sent for approval or you are not the requestor.';
    ...
    procedure addOtherCosts(employeeNo: Code[50]; imprestNo: Code[50]; voteItem: Text; requiredFor: Text; unitCost: Decimal; teamMember: Code[30]) status: Text
    var
    ...
    procedure createImprestSurrender(employeeNo: Code[50]; imprestNo: Code[50]; SurrenderNo: Code[50]; region: Code[10]) status: Text
    begin
    ...
    procedure cancelImprestSurrenderApproval(employeeNo: Code[50]; imprestSurrenderNo: Code[50]) status: Text
    var
        TEXT012: label 'The Requestor Must Be Part Of the Project  Team on The Memo';
        ProjectTeam: Record "Project Members";
        ImprestMemo1: Record "Imprest Memo";
    ...
    begin
        ...
        if Payments.FindSet then begin
            VarVariant := Payments;
            CustomApprovals.OnCancelDocApprovalRequest(VarVariant);
            status := 'success*You ghave successfully cancelled your approval request.';
        end else begin
            status := 'danger*An imprest surrender with the given number does not exist, you are not the requestor or is no longer open';
        end;
        exit(status);
    ...
    procedure updateSurrenderLineConf(employeeNo: Code[50]; imprestSurrenderNo: Code[50]; lineNo: Integer; amountSpent: Decimal; receipt: Text) status: Text
    var
        ImprestLines: Record "Imprest Lines Conf";
    begin
        status := 'danger*The imprest line could not be updated';
        Payments.Reset;
        Payments.SetRange("Account No.", employeeNo);
    ...
    procedure updateSurrenderLineAir(employeeNo: Code[50]; imprestSurrenderNo: Code[50]; lineNo: Integer; amountSpent: Decimal; receipt: Text) status: Text
    var
        ImprestLines: Record "Imprest Lines AirFares";
    begin
        status := 'danger*The imprest line could not be updated';
        Payments.Reset;
        Payments.SetRange("Account No.", employeeNo);
    ...
    procedure updateSurrenderLineComm(employeeNo: Code[50]; imprestSurrenderNo: Code[50]; lineNo: Integer; amountSpent: Decimal; receipt: Text) status: Text
    var
        ImprestLines: Record "Imprest Lines Comms";
    begin
        status := 'danger*The imprest line could not be updated';
        Payments.Reset;
        Payments.SetRange("Account No.", employeeNo);
    ...
    procedure createPurchaseStoreRequisition(employeeNo: Code[50]; requisitionNo: Code[50]; description: Text; type: Integer) status: Text
    var
        myType: Text;
    ...
    begin
        ...
        if requisitionNo = '' then begin //new  requisition
            ...
            PurchaseHeader."No." := noSeriesCodeunit.GetNextNo('STREQ', WorkDate, true);
            PurchaseHeader."Requester ID" := UserId;
            PurchaseHeader."Request-By No." := employeeNo;
            PurchaseHeader.Validate("Request-By No.");
            //  PurchaseHeader."Location Code" := location;
    ...
    procedure createRequisitionLine(employeeNo: Code[250]; requisitionNo: Code[250]; item: Code[250]; quantity: Decimal; directUnitCost: Decimal) status: Text
    var
    ...
    procedure createPurchaseRequisitionLine(employeeNo: Code[250]; requisitionNo: Code[250]; item: Code[250]; quantity: Decimal; directUnitCost: Decimal) status: Text
    var
        PurchaseLine: Record "Purchase Line";
    begin
        status := 'danger*Your Requisition Line could not be added';
    ...
    procedure fnDeletePurchaseLine(purchaseNo: Code[20]; lineNo: Integer) status: Text
    var
        PurchaseLine: Record "Purchase Line";
    ...
    begin
        ...
        PurchaseLine.Reset();
        PurchaseLine.setrange("Document No.", purchaseNo);
        PurchaseLine.setrange("Line No.", lineNo);
        If PurchaseLine.findfirst() then begin
            if PurchaseLine.Delete(true) then begin
                status := 'success*Your purchase line was added successfully';
            end else begin
                status := 'danger*Sorry, an error occurred while processing your request. Kindly try again';
            end;
        end else begin
            status := 'danger*Sorry, you could not find the purchase line. Kindly contact the administrator if this error persists.';
        end;
    ...
    begin
        ...
        if PurchaseHeader.FindSet then begin
            ...
            if ProcSetup."Enforce Strict Proc Budget che" = true then begin
                if PurchaseHeader."Total PRN Amount (LCY)" >= PurchaseHeader."PP Total Available Budget" then
                    Error('There is no enough Budget for No %1 Line No %2 Budget Available %3', PurchaseHeader."Procurement Plan ID", PurchaseHeader."Procurement Plan Entry No", PurchaseHeader."PP Total Available Budget");
            end;
    ...
    procedure cancelPurchaseRequisitionApproval(employeeNo: Code[50]; requisitionNo: Code[50]) status: Text
    var
        TEXT012: label 'The Requestor Must Be Part Of the Project  Team on The Memo';
        PurchaseHeader1: Record "Purchase Header";
        ProcSetup: Record "Procurement Setup";
    ...
    //         if TransportRequisition.Modify(true) then begin
    //             status := 'success*Your approval request was sent successfully';

    //         end else begin
    //             status := 'danger*Your approval request could not be sent. Kindly try again!';
    //         end;
    ...
    begin
        ...
        if recordType = 'leave' then begin
            ...
            if leaveApplications.FindSet then begin
                ...
                leaveApplications.TestField(Status, leaveApplications.Status::"Pending Approval");
            ...
            if recordType = 'staff claim' then begin
                ...
                if Payments.FindSet then begin
                    ...
                    if Payments.modify(true) then begin
                        status := 'success*Your staff claim has been cancelled successfully. ';

                    end else begin
                        status := 'danger*Your staff claim has not been cancelled. kindly try again!';
        ...
        //         end else begin
        //             status := 'danger*Sorry, the requisition could not be cancelled. Kindly try again.';
        //         end;
    ...
    begin
        ...
        if Payments.FindSet then begin
            ...
            // Payments.TestField(Job);
            // Payments.TestField("Job Task No");
            // // Payments.TestField("Surrender  Doc. No");
            // /*TESTFIELD(Job);
            ...
            TotalImprestAmount := Payments."Total Amount LCY";
            ...
            if Payments2.Get(staffClaim) then begin
                if Payments2.Status = Payments2.Status::"Pending Approval" then begin
                    ...
                    status := 'danger*Sorry, your staff claim could not be sent for approval. Kindly try again';
                end;
    ...
    procedure cancelStaffClaimApproval(employeeNo: Code[50]; staffClaim: Code[50]) status: Text
    var
        TEXT012: label 'The Requestor Must Be Part Of the Project  Team on The Memo';
        ProjectTeam: Record "Project Members";
        ImprestMemo1: Record "Imprest Memo";
    ...
    begin
        ...
        if Payments.FindSet then begin
            ...
            VarVariant := Payments;
            CustomApprovals.OnCancelDocApprovalRequest(VarVariant);
            status := 'success*You have successfully cancelled your staff claim.';
    ...
    procedure createStaffClaim(employeeNo: Code[50]; claimNo: Code[50]; paymentNarration: Text; fundCode: Code[100]; job: Code[100]; jobTask: Code[100]; surrenderNo: Text; programCode: code[50]; currency: code[50]; expenseType: Code[50]) status: Text
    begin
        ...
        if claimNo = '' then begin //new imprest surrender
            ...
            Payments.Validate("Payment Narration");
            Payments."Shortcut Dimension 1 Code" := fundCode;
            Payments.Validate("Shortcut Dimension 1 Code");
            Payments.Job := job;
            Payments.Validate(Job);
            ...
            Payments.Validate("Job Task No.");
            if surrenderNo <> '' then
                Payments."Surrender  Doc. No" := surrenderNo;
            Payments."Shortcut Dimension 2 Code" := programCode;
            Payments.Validate("Shortcut Dimension 2 Code");
            Payments."Currency Code" := currency;
            Payments.Validate("Currency Code");
            Payments."Type of Expense" := expenseType;
            Payments.Validate("Type of Expense");
            if Payments.Insert(true) then begin
            ...
            if Payments.FindSet then begin
                ...
                if surrenderNo <> '' then
                    Payments."Surrender  Doc. No" := surrenderNo;
                if Payments.Modify(true) then begin
                    status := 'success*Your staff claim was successfully updated*' + Payments."No.";
                end else begin
                    status := 'danger*Your staff claim could not be updated*' + Payments."No.";
                end;
    ...
    procedure updateTimesheetLines(tmHeader: Code[250]; lineNo: Integer; noOfHours: Decimal) status: Text
    var
        TimesheetLines: Record "Timesheet Lines";
    begin
        TimesheetLines.Reset();
        TimesheetLines.SetRange("Timesheet Header No", tmHeader);
        TimesheetLines.SetRange("Line No", lineNo);
        if TimesheetLines.FindSet() then begin
            TimesheetLines."Total Hours" := noOfHours;

            if TimesheetLines.Modify() then begin
                status := 'success*No of Hours Inserted Successfully.';
            end else begin
                status := 'danger*Failed to update.';
            end;
    ...
    procedure fnDeleteStaffClaimLine(claimNo: Code[20]; lineNo: Integer) status: Text
    var
        PVLines: Record "PV Lines";
    ...
    begin
        ...
        If PVLines.findfirst() then begin
            if PVLines.Delete(true) then begin
                status := 'success*Your staff claim line was added successfully';
            end else begin
                status := 'danger*Sorry, an error occurred while processing your request. Kindly try again';
            end;
        end else begin
            status := 'danger*Sorry, you could not find the staff claim line. Kindly contact the administrator if this error persists.';
        end;
    ...
    //         FuelMaintenanceRequisition.Status := FuelMaintenanceRequisition.Status::"Pending Approval";
    //         if FuelMaintenanceRequisition.Modify(true) then begin
    //             status := 'success*The Vehicle Maintenance request was successfully sent for approval';
    //         end else begin
    //             status := 'danger*Sorry, the vehicle Maintenance approval request could not be sent. Kindly try again later!';
    ...
    begin
        ...
        if StrategyWorkplanLines.FindSet then begin
            repeat
                AnnualLines := AnnualLines + '*' + StrategyWorkplanLines."Activity ID" + '*' + StrategyWorkplanLines.Description + '*' + StrategyWorkplanLines.Description + ':';
        ...
        if leaveNo = '' then begin
            ...
            if tbl_leaveApplications.FindSet(true) then begin
                ...
                If tbl_leaveApplications.Insert(true) then begin
                    ...
                    status := 'danger*The leave application was unsuccessful. Kindly try again later';
                END;
            end;
        end ELSE BEGIN
            tbl_leaveApplications.RESET;
    ...
    // [InherentPermissions(PermissionObjectType::Codeunit, Database::approval, 'rimd', InherentPermissionsScope::Both)]
    ...
    begin
        ...
        IF leaveApplications.FINDSET THEN BEGIN
            VarVariant := leaveApplications;
            CustomApprovals.OnCancelDocApprovalRequest(VarVariant);
            status := 'success*Your leave application was cancelled';
    ...
    procedure fncreatePurchaseRequisition(employeeNo: Code[50]; requisitionNo: Text; priorityLevel: Integer; itemcategory: Text; job: Text; jobtask: Text; requisitionProductGroup: Integer; description: Text; progCode: code[50]; fundingSource: code[50]; planEntry: Integer) status: Text
    var
        tbl_employee: Record Employee;
    ...
    begin
        ...
        IF Employee.FINDSET THEN BEGIN
            ...
            IF PurchaseHeader.FINDSET THEN BEGIN
                ...
                PurchaseHeader."Department Code" := tbl_employee.Department;
                ...
                PurchaseHeader."Procurement Plan Entry No" := planEntry;
                PurchaseHeader."Shortcut Dimension 2 Code" := progCode;
                PurchaseHeader.validate("Shortcut Dimension 2 Code");
                PurchaseHeader."Shortcut Dimension 1 Code" := fundingSource;
            ...
            END ELSE BEGIN
                ...
                PurchaseHeader."Requester ID" := tbl_userSetup."User ID";
                PurchaseHeader."Request-By No." := employeeNo;
                //PurchaseHeader.VALIDATE("Request-By No.");
                PurchaseHeader."Request-By Name" := tbl_employee."First Name" + ' ' + tbl_employee."Last Name";
                PurchaseHeader."Responsibility Center" := tbl_employee.Division;
    ...
    begin
        if docNo = '' then begin
            tbl_employee.reset;
            tbl_employee.SETRANGE("No.", empNo);
            IF tbl_employee.FINDSET THEN BEGIN
                tbl_payments.RESET;
                tbl_payments.INIT;
                tbl_payments."No." := '';
                tbl_payments."Account Type" := tbl_payments."Account Type"::Employee;
                tbl_payments."Payment Type" := tbl_payments."Payment Type"::"Petty Cash";
                tbl_payments."Document Type" := tbl_payments."Document Type"::"Petty Cash";
                tbl_payments."Account No." := pcashNo;
                tbl_payments."Account Name" := tbl_employee."First Name" + ' ' + tbl_employee."Last Name";
                tbl_payments.Payee := tbl_employee."First Name" + ' ' + tbl_employee."Last Name";
                tbl_payments."Payment Narration" := narration;

                // Amos Global Dimension 1 and 2 contain Division code and dept code respectively
                tbl_payments."Shortcut Dimension 1 Code" := tbl_employee."Global Dimension 1 Code";
                tbl_payments.Status := Payments.Status::Open;
                //Payments.VALIDATE("Shortcut Dimension 1 Code");
                tbl_payments."Shortcut Dimension 2 Code" := tbl_employee."Global Dimension 2 Code";
                tbl_payments."Department Code" := tbl_employee."Department Code";
                //Payments.VALIDATE("Shortcut Dimension 2 Code");
                IF tbl_payments.INSERT(TRUE) THEN BEGIN
                    status := 'success*Header created*' + tbl_payments."No.";
                END ELSE BEGIN
                    status := 'danger*Failed to create petty cash requisition*' + tbl_payments."No.";
                END
            END;
            ...
            tbl_payments.setrange("Account No.", pcashNo);
            ...
            if tbl_payments.findset(true) then begin
                ...
                IF tbl_employee.FINDSET THEN BEGIN
                    ...
                    tbl_payments."Department Code" := tbl_employee."Department Code";
                    //Payments.VALIDATE("Shortcut Dimension 2 Code");
                    IF tbl_payments.Modify(TRUE) THEN BEGIN
                        status := 'success*Header Modified*' + tbl_payments."No.";
                    END ELSE BEGIN
                        status := 'danger*Failed to modify petty cash requisition. Kindly try again.' + tbl_payments."No.";
                    END
                END;


            end else begin
                status := 'danger*Failed to modify petty cash requisition. Maybe you are not the requestor or it is no longer open. Contact the administrator if this error persists';
            end;
    ...
    // create imprest requisition
    procedure fnCreateImprestRequisition(requisitionNo: Code[20]; empNo: Code[20]; referenceNo: Text; paymentNarration: Text; project: Code[20]; projectTaskNo: Code[20]; fundingCode: Code[20]; donorCode: Code[20]; returnDate: Date; programCode: Code[20]) status: Text
    var
        tbl_payments: Record Payments;
        tbl_employee: Record Employee;
        tbl_customer: Text;
    begin
        if requisitionNo = '' then begin
            tbl_payments.Reset;
            tbl_payments.Init;
            tbl_payments."Account Type" := tbl_payments."Account Type"::Customer;
            tbl_employee.Reset();
            tbl_employee.SetRange("No.", empNo);
            If tbl_employee.FindFirst() Then begin
                tbl_payments."Account No." := tbl_employee."Advance Account";
                tbl_payments.validate("Account No.");
            end;
            tbl_payments."Reference No." := referenceNo;
            tbl_payments."Payment Type" := tbl_payments."Payment Type"::Imprest;
            tbl_payments."Payment Narration" := paymentNarration;
            tbl_payments."Travel Date" := returnDate;
            tbl_payments.Date := Today;
            tbl_payments.Job := project;
            tbl_payments."Job Task No." := projectTaskNo;
            //validation
            tbl_payments."Shortcut Dimension 1 Code" := fundingCode;
            tbl_payments."Shortcut Dimension 3 Code" := donorCode;
            tbl_payments."Shortcut Dimension 2 Code" := programCode;
            tbl_payments.validate("Shortcut Dimension 1 Code");
            tbl_payments.validate("Shortcut Dimension 2 Code");
            tbl_payments.validate("Shortcut Dimension 3 Code");
            tbl_payments.Status := tbl_payments.Status::Open;
            if tbl_payments.Insert(true) then begin
                status := 'success*Your advance request has been received successfully*' + tbl_payments."No.";
            end else begin
                status := 'danger*Sorry, an error occurred while processing your request. Kindly try again.';
            end;
        end else begin
            ...
            If tbl_employee.FindFirst() Then begin
                tbl_payments.reset;
                tbl_payments.setrange("Account No.", tbl_employee."Advance Account");
                tbl_payments.setrange("No.", requisitionNo);
                tbl_payments.setrange(Status, tbl_payments.Status::Open);
                if tbl_payments.findset(true) then begin
                    tbl_payments."Payment Narration" := paymentNarration;
                    tbl_payments."Reference No." := referenceNo;
                    ...
                    tbl_payments."Shortcut Dimension 2 Code" := programCode;
                    if tbl_payments.Modify(true) then begin
                        status := 'success*Your advance request has been modified successfully*' + tbl_payments."No.";
                    end else begin
                        status := 'danger*Sorry, an error occurred while modifying your request. Kindly try agaun';
                    end;
                end else begin
                    status := 'danger*Sorry, the requisition does not belong to you or is not open. Kindly contact the administrator.';
                end;
            end else begin
                status := 'danger*Sorry, you dont have  an Advance Account.Kindly contact ICT';
            end;
    ...
    // populate imprest lines
    procedure fnCreateImprestLine(empNo: Code[20]; imprestNo: Code[20]; accountNo: Integer; purpose: Text; countryCode: Code[100]; areaCode: Code[100]; startDate: Date; endDate: Date; ImprestAmount: Decimal; currencyCode: Code[50]) status: Text
    var
        tbl_payments: Record Payments;
    ...
    begin
        // check whether the advance request exist for the employee and the request is still open
        tbl_employee.Reset();
        tbl_employee.SetRange("No.", empNo);
        If tbl_employee.FindFirst() Then begin
            tbl_payments.Reset();
            ...
            tbl_payments.setrange(Status, tbl_payments.Status::Open);
            if tbl_payments.findset(true) then begin
                tbl_imprestLines.init;
                tbl_imprestLines."Account Type" := tbl_imprestLines."Account Type"::"G/L Account";
                // account No 1 -- is local travel, account No 2 is international travel
                if accountNo = 1 then
                    tbl_imprestLines."Account No." := Format(5710);
                if accountNo = 2 then
                    tbl_imprestLines."Account No." := Format(5720);
                if accountNo = 3 then
                    tbl_imprestLines."Account No." := Format(5730);
                tbl_imprestLines.Validate("Account No.");
                tbl_imprestLines."Daily Rate" := ImprestAmount;
                tbl_imprestLines.purpose := purpose;
                tbl_imprestLines."Country Code" := countryCode;
                tbl_imprestLines."Area Code" := areaCode;
                tbl_imprestLines."Start Date" := startDate;
                tbl_imprestLines.No := imprestNo;
                tbl_imprestLines."End Date" := endDate;
                tbl_imprestLines.validate("End Date");
                tbl_imprestLines."Currency Code" := currencyCode;
                tbl_imprestLines.validate("Currency Code");
                if tbl_imprestLines.Insert(true) then begin
                    status := 'success*Your imprest line was added successfully';
                end else begin
                    status := 'danger*Sorry, an error occurred while processing your request. Kindly try again';
                end;

            end else begin
                status := 'danger*Sorry, you could not add an imprest line. The advance request is either not open or you are not the requestor. Kindly contact the administrator if this error persists.';
            end;
        end else begin
            status := 'danger*Sorry, you dont have  an Advance Account.Kindly contact ICT';
        end;
    ...
    // populate imprest lines
    procedure fnCreateImprestLineConf(empNo: Code[20]; imprestNo: Code[20]; accountNo: Code[20]; purpose: Text; countryCode: Code[100]; areaCode: Code[100]; startDate: Date; endDate: Date; ImprestAmount: Decimal; currencyCode: Code[50]) status: Text
    var
        tbl_payments: Record Payments;
    ...
    begin
        ...
        If tbl_employee.FindFirst() Then begin
            ...
            if tbl_payments.findset(true) then begin
                ...
                tbl_imprestLines."Account Type" := tbl_imprestLines."Account Type"::"G/L Account";
                // account No 1 -- is local travel, account No 2 is international travel
                tbl_imprestLines."Account No." := accountNo;
                // if accountNo = 1 then
                //     tbl_imprestLines."Account No." := Format(45900);
                // if accountNo = 2 then
                //     tbl_imprestLines."Account No." := Format(45950);
                tbl_imprestLines.Validate("Account No.");
                tbl_imprestLines."Daily Rate" := ImprestAmount;
                tbl_imprestLines.purpose := purpose;
                tbl_imprestLines."Country Code" := countryCode;
    ...
    procedure fnCreateImprestLineAir(empNo: Code[20]; imprestNo: Code[20]; accountNo: Code[20]; purpose: Text; countryCode: Code[100]; areaCode: Code[100]; startDate: Date; endDate: Date; ImprestAmount: Decimal; currencyCode: Code[50]) status: Text
    var
        tbl_payments: Record Payments;
    ...
    procedure fnCreateImprestLineComm(empNo: Code[20]; imprestNo: Code[20]; accountNo: Code[20]; purpose: Text; countryCode: Code[100]; areaCode: Code[100]; startDate: Date; endDate: Date; ImprestAmount: Decimal; currencyCode: Code[50]) status: Text
    var
        tbl_payments: Record Payments;
    ...
    procedure fnDeleteImprestLine(imprestNo: Code[20]; lineNo: Integer) status: Text
    var
        tbl_payments: Record Payments;
        tbl_imprestLines: Record "Imprest Lines";
    ...
    begin
        ...
        tbl_imprestLines.setrange(No, imprestNo);
        tbl_imprestLines.setrange("Line No", lineNo);
        If tbl_imprestLines.findfirst() then begin
            if tbl_imprestLines.Delete(true) then begin
                status := 'success*Your imprest line was added successfully';
            end else begin
                status := 'danger*Sorry, an error occurred while processing your request. Kindly try again';
            end;
        end else begin
            status := 'danger*Sorry, you could not find the imprest line. Kindly contact the administrator if this error persists.';
        end;
    ...
    procedure fnDeleteImprestLineConf(imprestNo: Code[20]; lineNo: Integer) status: Text
    var
        tbl_payments: Record Payments;
        tbl_imprestLines: Record "Imprest Lines Conf";
    ...
    procedure fnDeleteImprestLineAir(imprestNo: Code[20]; lineNo: Integer) status: Text
    var
        tbl_payments: Record Payments;
        tbl_imprestLines: Record "Imprest Lines AirFares";
    ...
    procedure fnDeleteImprestLineComm(imprestNo: Code[20]; lineNo: Integer) status: Text
    var
        tbl_payments: Record Payments;
        tbl_imprestLines: Record "Imprest Lines Comms";
    ...
    // create timesheets
    procedure fnCreateTimesheet(resourceNo: Code[30]; startingDate: Date; weeks: Integer) data: text
    var
        tbl_timeSheet: Record "Time Sheet Header";
        endDate: Date;
        myWeekCounter: Integer;
        currentDate: Date;
        previousTimeSheetDate: Date;
    ...
    begin
        ...
        for myWeekCounter := 1 to weeks Do begin
            tbl_timeSheet.Init();
            tbl_timeSheet."No." := NoSeriesMgt.GetNextNo('TSN', WorkDate, true); //Important --> edit the value of 'TSN' to fit the current time sheet number series code
            tbl_timeSheet."Resource No." := resourceNo;
            tbl_timeSheet.Validate("Resource No.");
            tbl_timeSheet."Starting Date" := currentDate;
            tbl_timeSheet."Unit of Measure" := 'DAY';
            tbl_timeSheet."Ending Date" := currentDate + 6;
            currentDate := tbl_timeSheet."Ending Date" + 1; // keep counter of the dates
            if tbl_timeSheet.insert(true) then begin

                data := 'success*Your timesheet has been created successfully';
            end else begin
                data := 'danger*There was an error creating your timesheet, kindly try again later. Contact the administrator if this error persists';
            end;
        end;
    ...
    procedure fnPopulateTimeSheetLines(resourceNo: Code[30]; timeSheetHeader: Code[30]; customer: Code[30]; description: Text; workHours: array[7] of Integer) data: Text;
    var
    ...
    begin
        ...
        tbl_timesheetHeader.setrange("No.", timeSheetHeader);
        ...
        if tbl_timesheetHeader.findset(true) then begin
            ...
            tbl_timeSheetLine.setrange("Time Sheet No.", timeSheetHeader);
            ...
            tbl_timeSheetLine.Customer := customer;
            tbl_timeSheetLine.Description := description;
            tbl_timeSheetLine."Time Sheet No." := timeSheetHeader;
            tbl_timeSheetLine.Type := tbl_timeSheetLine.Type::Resource;
            ...
            if tbl_timeSheetLine.insert(true) then begin
                for myHours := 1 to 7 do begin
                    ...
                    tbl_timeSheetLineDetail."Time Sheet No." := timeSheetHeader;
                    tbl_timeSheetLineDetail."Time Sheet Line No." := tbl_timeSheetLine."Line No.";
                    tbl_timeSheetLineDetail.Date := myDate;
                    tbl_timeSheetLineDetail.Quantity := workHours[myHours];
                    tbl_timeSheetLineDetail.Type := tbl_timeSheetLineDetail.Type::Resource;
                    myDate += 1;
                    tbl_timeSheetLineDetail.insert(true);

                end;
                data := 'success*Your Time sheet line was added successfully';
            end else begin
                data := 'danger*Sorry, there was an error creating your timesheet line. Kindly try again. Contact the administrator if this error persists';
            end;
        end else begin
            data := 'danger*Sorry, you seem not to be the owner of the time sheet. Contact the administrator if this is not the case';
        end;

    end;

    procedure fnPopulateTimeSheetLinesJob31(resourceNo: Code[30]; timeSheetHeader: Code[30]; job: Code[30]; jobTask: Code[30]; percDistribution: Decimal; workHours: array[31] of Integer) data: Text;
    var
        tbl_timesheetHeader: Record "Time Sheet Header";
        tbl_timeSheetLine: Record "Time Sheet Line";
        tbl_timeSheetLineDetail: Record "Time Sheet Detail";
    ...
    procedure fnPopulateTimeSheetLinesJob30(resourceNo: Code[30]; timeSheetHeader: Code[30]; job: Code[30]; jobTask: Code[30]; percDistribution: Decimal; workHours: array[30] of Integer) data: Text;
    var
        tbl_timesheetHeader: Record "Time Sheet Header";
        tbl_timeSheetLine: Record "Time Sheet Line";
        tbl_timeSheetLineDetail: Record "Time Sheet Detail";
    ...
    procedure fnPopulateTimeSheetLinesJob29(resourceNo: Code[30]; timeSheetHeader: Code[30]; job: Code[30]; jobTask: Code[30]; percDistribution: Decimal; workHours: array[29] of Integer) data: Text;
    var
        tbl_timesheetHeader: Record "Time Sheet Header";
        tbl_timeSheetLine: Record "Time Sheet Line";
        tbl_timeSheetLineDetail: Record "Time Sheet Detail";
    ...
    procedure fnPopulateTimeSheetLinesJob28(resourceNo: Code[30]; timeSheetHeader: Code[30]; job: Code[30]; jobTask: Code[30]; percDistribution: Decimal; workHours: array[28] of Integer) data: Text;
    var
        tbl_timesheetHeader: Record "Time Sheet Header";
        tbl_timeSheetLine: Record "Time Sheet Line";
        tbl_timeSheetLineDetail: Record "Time Sheet Detail";
    ...
    procedure fnPopulateTimeSheetLinesJob(resourceNo: Code[30]; timeSheetHeader: Code[30]; job: Code[30]; jobTask: Code[30]; workHours: array[7] of Integer) data: Text;
    var
        tbl_timesheetHeader: Record "Time Sheet Header";
        tbl_timeSheetLine: Record "Time Sheet Line";
        tbl_timeSheetLineDetail: Record "Time Sheet Detail";
    ...
    procedure fnDeleteTimesheetSheetLinesJob(timeSheetHeader: Code[30]; lineNo: Integer) data: Text;
    var
        tbl_timesheetHeader: Record "Time Sheet Header";
        tbl_timeSheetLine: Record "Time Sheet Line";
        tbl_timeSheetLineDetail: Record "Time Sheet Detail";
    ...
    begin
        ...
        tbl_timeSheetLine.Reset();
        tbl_timeSheetLine.setrange("Time Sheet No.", timeSheetHeader);
        tbl_timeSheetLine.setrange("Line No.", lineNo);
        If tbl_timeSheetLine.findfirst() then begin
            if tbl_timeSheetLine.Delete(true) then begin
                data := 'success*The timesheet line has been deleted successfully.';
            end else begin
                data := 'danger*Sorry, there was an error deleting your time sheet line. Kindly try again. Contact the administrator if this error persists';
            end;
        end else begin
            data := 'danger*Document could not be found';
        end;


    end;

    procedure fnSubmitTimesheetSheetLinesJob(timeSheetHeader: Code[30]) data: Text;
    var
        tbl_timesheetHeader: Record "Time Sheet Header";
        tbl_timeSheetLine: Record "Time Sheet Line";
        tbl_timeSheetLineDetail: Record "Time Sheet Detail";
    ...
    begin
        ...
        if tbl_timeSheetLine.findset(true) then begin
            repeat
                TimeSheetApprovalMgt.Submit(tbl_timeSheetLine);
            until tbl_timeSheetLine.Next() = 0;
            data := 'success*The timesheet has been submitted successfully.';
        end else begin
            data := 'danger*Document could not be found';
        end;
    ...
    procedure fnSendTimeSheetApproval2(timeSheetHeader: Code[30]) data: Text;
    var
        tbl_timesheetHeader: Record "Time Sheet Header";
        tbl_timeSheetLine: Record "Time Sheet Line";
        tbl_timeSheetLineDetail: Record "Time Sheet Detail";
    ...
    begin
        ...
        if TimesheetHeader_tbl.findset(true) then begin
            ...
            if tbl_timeSheetLine.FindSet() then begin
                ...
                if tbl_timeSheetLine."Distribution Percentage" <> 100 then
                    Error('The total distribution percentage in a timesheet should add up to 100. Please check your lines.');
            end;
            VarVariant := TimesheetHeader_tbl;
            IF CustomApprovals.CheckApprovalsWorkflowEnabled(VarVariant) THEN
                CustomApprovals.OnSendDocForApproval(VarVariant);

            data := 'success*Your timesheet was successfully  sent for approval';
        end else begin
            data := 'danger*Document could not be found';
        end;
    end;

    procedure fnSendTimesheetApproval(timesheetNo: Code[30]) data: Text
    var
        tbl_TimeSheetLine: Record "Time Sheet Line";
    ...
    begin
        ...
        employeeTimesheet.setrange("Document No.", timesheetNo);
        if employeeTimesheet.FindFirst() then begin
            VarVariant := employeeTimesheet;
            If CustomApprovals.CheckApprovalsWorkflowEnabled(VarVariant) then
                CustomApprovals.OnSendDocForApproval(VarVariant);
            data := 'success*Your timesheet was successfully  sent for approval';
        end else begin
            data := 'danger*Sorry, Your timesheet could not be found. Kindly contact the administrator';
        end;
    end;

    procedure fncancelTimesheetApproval(timesheetNo: Code[30]) data: Text
    var
        tbl_TimeSheetLine: Record "Time Sheet Line";
        timeSheetApprovalMgmt: Codeunit "Time Sheet Approval Management";
        tbl_timesheetHeader: Record "Time Sheet Header";
    ...
    begin
        ...
        employeeTimesheet.setrange("Document No.", timesheetNo);
        if employeeTimesheet.FindFirst() then begin
            employeeTimesheet.TestField(Status, employeeTimesheet.Status::"Pending Approval");
            VarVariant := employeeTimesheet;
            CustomApprovals.OnCancelDocApprovalRequest(VarVariant);
            data := 'success*Your timesheet was successfully  sent for approval';
        end else begin
    ...
    procedure InsertDocumentLink(appNo: Code[20]; fileName: Text; fileLink: Text) data: Text
    var
        tbl_TimeSheetLine: Record "Time Sheet Line";
        timeSheetApprovalMgmt: Codeunit "Time Sheet Approval Management";
        RecordLink: Record "Record Link";
    ...
    begin
        uploadedSharepointDocs.Init();
        uploadedSharepointDocs.ApplicationNo := appNo;
        uploadedSharepointDocs.FileName := fileName;
        uploadedSharepointDocs.fileLink := fileLink;
        if uploadedSharepointDocs.insert(true) then begin

            data := 'success*The document has been uploaded successfully.';
        end else begin
            data := 'danger*Sorry, there was an error uploding your document. Kindly try again. Contact the administrator if this error persists';
        end;

    end;

    procedure DeleteDocumentLink(appNo: Code[20]; entryNo: Integer) data: Text
    var
        tbl_TimeSheetLine: Record "Time Sheet Line";
        timeSheetApprovalMgmt: Codeunit "Time Sheet Approval Management";
    ...
    begin
        ...
        uploadedSharepointDocs.setrange(EntryNo, entryNo);
        If uploadedSharepointDocs.findfirst() then begin
            if uploadedSharepointDocs.Delete(true) then begin
                data := 'success*The document has been deleted successfully.';
            end else begin
                data := 'danger*Sorry, there was an error deleting your document. Kindly try again. Contact the administrator if this error persists';
            end;
        end else begin
            data := 'danger*Document could not be found';
        end;
