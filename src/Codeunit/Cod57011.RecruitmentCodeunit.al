...
Codeunit 57011 "eRecruitment"
{
    ...
    procedure FnSendIPPassword(emailaddress: Text; DocumentNo: Code[20]) passChangestatus: Text
    var
        DynasoftPortalUser: Record "Dynasoft Portal User";
        RandomDigit: Text;
        Body: Text;
    begin
        ...
        RandomDigit := DelChr(RandomDigit, '=', '{}-01');
        RandomDigit := CopyStr(RandomDigit, 1, 8);
        if sendPass(RandomDigit, emailaddress) then begin
            DynasoftPortalUser.Init();
            DynasoftPortalUser."Password Value" := RandomDigit;
            DynasoftPortalUser."Last Modified Date" := Today;
            DynasoftPortalUser."Change Password" := false;
            DynasoftPortalUser."User Name" := emailaddress;
            DynasoftPortalUser."Authentication Email" := emailaddress;
            DynasoftPortalUser."Record ID" := DocumentNo;
            DynasoftPortalUser."Record Type" := DynasoftPortalUser."record type"::"Implementing partner";
            if DynasoftPortalUser.Insert(true) then begin
                //passChangestatus := 'danger*The Password was Not Modified';
            end else begin
                //passChangestatus := 'danger*The Password was Not Modified';
            end;
    ...
    procedure sendPass(Pass: Text; EAddrr: Text): Boolean
    var
        Recepient: Text;
    begin
        Body := '';
        Body := Pass;
        EmailMsg.Create(EAddrr, 'OTP', Body);
        exit(Email.Send(EmailMsg));
    end;
    ...
    begin
        ...
        Email.Send(EmailMsg, EmailScenario::HR);
