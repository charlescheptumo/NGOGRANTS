...
Table 69006 "DeductionsX"
{
    ...
    fields
    {
        ...
        field(83; "Account No."; Code[20])
        {
            ...
            //         "Account Type"::Vendor:
            //             begin
            //                 if Ven.Get("Account No.") then begin
            //                     "Account Name" := Ven.Name;
            //                     Payee := Ven.Name;
            //                 end;
            //             end;
        ...
        // May
        field(50005; "SHIF Code"; Boolean)
        {
            DataClassification = ToBeClassified;
            trigger OnValidate()
            begin
                if Confirm('Would you like to update SHIF Code for all employees with this Code', true) then begin
                    AssignmentMatrixX.Reset;
                    AssignmentMatrixX.SetRange(Type, AssignmentMatrixX.Type::Deduction);
                    AssignmentMatrixX.SetRange(Code, Code);
                    //       AssignmentMatrixX.SetRange(Closed, false);
                    if AssignmentMatrixX.FindSet then begin
                        repeat
                            AssignmentMatrixX."SHIF Code" := "SHIF Code";
                            AssignmentMatrixX.Modify;
                        until AssignmentMatrixX.Next = 0;
                    end;
                end else
                    Message('SHIF not updated to all employees');
            end;
        }
        field(50006; "Housing Fund Code"; Boolean)
        {
            DataClassification = ToBeClassified;
            trigger OnValidate()
            begin
                if Confirm('Would you like to update Housing Fund Code for all employees with this Code', true) then begin
                    AssignmentMatrixX.Reset;
                    AssignmentMatrixX.SetRange(Type, AssignmentMatrixX.Type::Deduction);
                    AssignmentMatrixX.SetRange(Code, Code);
                    //       AssignmentMatrixX.SetRange(Closed, false);
                    if AssignmentMatrixX.FindSet then begin
                        repeat
                            AssignmentMatrixX."Housing Fund Code" := "Housing Fund Code";
                            AssignmentMatrixX.Modify;
                        until AssignmentMatrixX.Next = 0;
                    end;
                end else
                    Message('Housing Fund not updated to all employees');
            end;
        }
        field(50007; "NSSF"; Boolean)
        {
            DataClassification = ToBeClassified;
            trigger OnValidate()
            begin
                if Confirm('Would you like to update NSSF Code for all employees with this Code', true) then begin
                    AssignmentMatrixX.Reset;
                    AssignmentMatrixX.SetRange(Type, AssignmentMatrixX.Type::Deduction);
                    AssignmentMatrixX.SetRange(Code, Code);
                    //       AssignmentMatrixX.SetRange(Closed, false);
                    if AssignmentMatrixX.FindSet then begin
                        repeat
                            AssignmentMatrixX."NSSF Code" := "NSSF";
                            AssignmentMatrixX.Modify;
                        until AssignmentMatrixX.Next = 0;
                    end;
                end else
                    Message('NSSF not updated to all employees');
            end;
        }
    }
