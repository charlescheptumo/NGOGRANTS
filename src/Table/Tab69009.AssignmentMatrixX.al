...
Table 69009 "Assignment Matrix-X"
{
    ...
    fields
    {
                    ...
                    if Payments.Get(Code) then begin
                        ...
                        if Payments."Calculation Method" = Payments."calculation method"::"% of Basic pay" then begin
                            if Empl.Get("Employee No") then begin
                                ...
                                // Empl.CalcFields(Basic, Empl.Basic);
                                ...
                                // Amount := (Payments.Percentage / 100 * (Empl.Basic)) - Payments."Deduct From Pension";
                        ...
                        if Payments."Calculation Method" = Payments."calculation method"::"% of Gross pay" then begin
                            if Empl.Get("Employee No") then begin
                                ...
                                //Amount := ((Payments.Percentage / 100) * (Empl."Total Allowances1"));
                                // Message('Empl.Basic=%1 Empl."Total Allowances1"=%2', Empl.Basic, Empl."Total Allowances1");
                                Amount := PayrollRounding(Amount);//round
                            end;
                        end;
                    ...
                    if Deductions.Get(Code) then begin
                        ...
                        if Deductions."Calculation Method" = Deductions."calculation method"::"% of Basic Pay" then begin
                            if Empl.Get("Employee No") then begin
                                ...
                                //if it is pension then employee contribution to pick percentage from the assignment matrix percentage field.
                                if Deductions."Pension Scheme" then begin
                                    Amount := Rec.Percent / 100 * Empl.Basic;
                                end else
        ...
        field(65006; "Percent"; Decimal)
        {
            //Field used to allow manual entry of percent deduction of pension for employee
            trigger OnValidate()
            var
            ...
            begin
                Deduction.Reset();
                Deduction.SetRange(Code, Rec.Code);
                Deduction.SetRange("Pension Scheme", true);
                if Deduction.FindFirst() then begin
                    if Empl.Get("Employee No") then begin
                        Empl.SetRange(Empl."Pay Period Filter", "Payroll Period");
                        Empl.CalcFields(Basic, Empl."Basic Arrears");
                        // Earning.SetRange("Basic Salary Code", true);
                        // if Earning.FindFirst() then begin
                        //     AssMat.Reset();
                        //     AssMat.SetRange("Employee No", "Employee No");
                        //     AssMat.SetRange(Code, Earning.Code);
                        //     AssMat.SetRange("Payroll Period", "Payroll Period");
                        //     AssMat.SetRange(Type, AssMat.Type::Payment);
                        //     if AssMat.FindFirst() then
                        // Amount := AssMat.Amount * Percent / 100;
                        Amount := Empl.Basic * Percent / 100;
                        Validate(Amount);
        ...
        field(65007; "SHIF Code"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(65008; "Housing Fund Code"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(65009; "Pension Fund Code"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(65010; "Pensionable Earnings"; Decimal)
        {
            DataClassification = ToBeClassified;
        }
        field(65011; "NSSF Code"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
