...
Table 57008 "Imprest Memo"
{
    ...
    begin
        ...
        if "Document Type" = "document type"::"Imprest Memo" then begin
            ImprestMemo.Reset;
            ImprestMemo.SetRange(ImprestMemo.Posted, false);
            ImprestMemo.SetRange(ImprestMemo."User ID", "User ID");
            //IF LoansR."Client Name"='' THEN BEGIN
            if ImprestMemo.Count > 1 then begin
                Error('You still have Untilised some Open Imprest Memo. Please utilise them first');
            end;
        end;
        ...
        if "Document Type" = "document type"::"Imprest Memo" then begin
            ...
            if "Document Type" = "document type"::"Imprest Memo" then begin
                ...
                if CustRec.Find('-') then begin
                    //CustRec.CALCFIELDS(Amount);
                    if CustRec.Amount > 0 then
                        Error(Text007, CustRec.Amount);
                end;
        ...
        */
        ...
        if UsersRec.Get(UserId) then begin
            if UsersRec."Portal User" = false then begin //exempt the portal user name
                Requestor := UsersRec."Employee No.";
            end;
        end;
